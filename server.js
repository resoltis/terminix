...
app.post('/generateFiles', urlencodedParser, function (req, res) {
  ...
  fs.appendFileSync(path.join('CTF Files', req.body.environment + '.params.json'), '[\n');
  ...
  function fileWriterJson(fileName, paramKey, paramValue) {
    fs.appendFileSync(path.join('CTF Files', fileName), '{' + '\n' + '"ParamaterKey": ' + '"' + paramKey + '",' + '\n');
    fs.appendFileSync(path.join('CTF Files', fileName), '"ParamaterValue": ' + '"' + paramValue + '"' + '\n' + '},\n');
    return
  }
